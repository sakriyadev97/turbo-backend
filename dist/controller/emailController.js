...
exports.sendIndividualOrderEmailWithPDF = exports.sendBulkOrderEmailWithPDF = exports.sendOrderEmail = void 0;
...
const sendOrderEmail = async (req, res) => {
    try {
        ...
        <ul>
          ${orders.map((order) => `<li><strong>Model:</strong> ${order.modelName}, <strong>Quantity:</strong> ${order.quantity}, <strong>Location:</strong> ${order.location}</li>`).join('')}
        </ul>
        <p>Thank you!</p>
      `
        };
...
const sendBulkOrderEmailWithPDF = async (req, res) => {
    try {
        ...
        <h3>Order Summary:</h3>
        <ul>
          ${orders.map((order) => `<li><strong>Part:</strong> ${order.partNumber} | <strong>Model:</strong> ${order.modelName || order.model} | <strong>Qty:</strong> ${order.quantity} | <strong>Location:</strong> ${order.location}</li>`).join('')}
        </ul>
...
exports.sendBulkOrderEmailWithPDF = sendBulkOrderEmailWithPDF;
const sendIndividualOrderEmailWithPDF = async (req, res) => {
    try {
        const { orders, orderNumber } = req.body;
        if (!orders || !Array.isArray(orders) || orders.length === 0) {
            return res.status(400).json({ error: 'Missing or invalid orders data' });
        ...
        try {
            ...
            pdfBuffer = await (0, pdfGenerator_1.generateBulkInvoicePDF)(orderData);
            console.log('PDF generated successfully, buffer size:', pdfBuffer.length);
            pdfGenerated = true;
        }
        catch (pdfError) {
        ...
        <h3>Order Details:</h3>
        <ul>
          ${orders.map((order) => `<li><strong>Part:</strong> ${order.partNumber || order.modelName} | <strong>Model:</strong> ${order.modelName || order.model} | <strong>Qty:</strong> ${order.quantity} | <strong>Location:</strong> ${order.location}</li>`).join('')}
        </ul>
...
exports.sendIndividualOrderEmailWithPDF = sendIndividualOrderEmailWithPDF;
